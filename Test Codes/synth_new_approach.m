%New Approach on synthetic data
%Neelabhro Roy
%IIIT-Delhi
...
%    load(pcaFile, 'W');
    ...
    %TestPCA = W' * TestSet';
    %X22 = TestPCA(1:50, 1:316);
    %X12 = TestPCA(1:50, 317:end);
    %X22 = TestPCA(:, 1:316);
    %X12 = TestPCA(:, 317:end);
...
%        U  = (( W1'*X1 * transpose(V1)) + ( W2'*X2 * transpose(V2))) .* inv((( V1 * transpose(V1)) + ( V2 * transpose(V2)) + (Lu*eye(k))));
        ...
        C1 = (X1*V1'*W1 + nu*X1*V1'*P1)/(eye(k) + nu*P1'*P1)+0.0001*eye(k);
        [Ua1,da1] = eig(A1 + 0.0001*eye(k));
        Ua1 = (Ua1);
        da1 = (da1);
        [Vb1,db1] = eig(B1 + 0.0001*eye(k));
        Vb1 = real(Vb1);
        db1 = real(db1);
        C1t = ((Ua1 +eye(k)*0.0001)\C1)*Vb1;
        
        for p = 1:d
        
            for q = 1:d
                W1t(p,q) = C1t(p,q) / ( da1(p,p) + db1(q,q)+.00001 );
            end
            
        end
        
        W1 = ((Ua1 +eye(k)*0.0001)*W1t) / (Vb1+eye(k)*0.0001);
        for j = 1:k
        ...
        C2 = (X2*V2'*W2 + nu*X2*V2'*P2)/(eye(k) + nu*P2'*P2)+0.0001*eye(k);
        [Ua2,da2] = eig(A2 + 0.0001*eye(k));
        Ua2 = (Ua2);
        da2 = (da2);
        [Vb2,db2] = eig(B2 + 0.0001*eye(k));
        Vb2 = real(Vb2);
        db2 = real(db2);
        C2t = ((Ua2 +eye(k)*0.0001)\C2)*Vb2;
        
        for p = 1:d
        
            for q = 1:d
                W2t(p,q) = C2t(p,q) / ( da2(p,p) + db2(q,q) +.00001);
            end
            
        end          

        W2 = ((Ua2 +eye(k)*0.0001)*W2t)/(Vb2+eye(k)*0.0001 );
        for j = 1:k
