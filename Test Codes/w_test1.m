...
%% Main algorithm
    for i = 1:100
        U  = (( W'*X1 * transpose(V1)) + ( W'*X2 * transpose(V2))) .* inv((( V1 * transpose(V1)) + ( V2 * transpose(V2)) + (Lu*eye(k))));
        V1 = inv(((transpose(U) * U) + (nu + beta + Lv) * eye(k))) * ((transpose(U) *W'* X1) + (beta* A * V2) + nu * P1 * W'*X1);
        V2 = inv(((transpose(U) * U) + ( beta * transpose(A) * A) + (nu + Lv) .* eye(k))) * ((transpose(U) *W'* X2) + (beta* transpose(A) * V1) + nu * P2*W' * X2);
        P1 = (V1 * transpose(W'*X1)) * inv((W'*X1 * transpose(W'*X1)) + (Lp/nu)*eye(k));
        P2 = (V2 * transpose(W'*X2)) * inv((W'*X2 * transpose(W'*X2)) + (Lp/nu)*eye(k));
